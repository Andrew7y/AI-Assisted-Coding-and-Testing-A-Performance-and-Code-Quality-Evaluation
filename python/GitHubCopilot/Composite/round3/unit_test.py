import pytest
from code_3 import Authentication, BalanceInquiry, Withdrawal, Deposit, PinChange, PrintReceipt, ATMComposite, Account

@pytest.fixture
def account():
    return Account(account_number="1234567890", balance=1000, pin="1234")

def test_authentication_success(account):
    auth = Authentication(account, pin="1234")
    auth.execute()
    assert auth.is_authenticated

def test_authentication_failure(account):
    auth = Authentication(account, pin="wrong_pin")
    auth.execute()
    assert not auth.is_authenticated

def test_authentication_max_attempts(account):
    auth = Authentication(account, pin="wrong_pin")
    for _ in range(account.max_attempts):
        auth.execute()
    assert not auth.is_authenticated
    assert account.attempts == account.max_attempts
